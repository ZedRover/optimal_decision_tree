...
using Printf
using JuMP
using CPLEX, COPT #, Gurobi#, SCIP, Ipopt
...
function var_c(m, l_c, c_udt, c_dt, c_w, K, T, Tb, rlx = false)
	...
	if length(c_dt) == 0
		...
		c = Matrix(undef, K, T) # the first Tb*k is useless, should consider if memory issue raises
...
function cons_setup_leaf_SG(m, y, n, K, T, Tb, c, z, z_udt, costs)
	...
	if z_udt isa Vector{Int64}# for only SG mode
		...
		#@constraint(m, [i in 1:n, t in (Tb+1):T], 0.5*sum(y[k,i]+c[k,t]-2*y[k,i]*c[k,t] for k in 1:K)-costs[i] <= 1-z[i,t-Tb])
	else
...
function optimizer_init(mute, time_lapse)
	if OPT_NAME == "CPLEX"
		...
		m = direct_model(COPT.Optimizer())
		if mute
			set_optimizer_attribute(m, "Logging", 0)
			set_optimizer_attribute(m, "LogToConsole", 0)
		end
		set_optimizer_attribute(m, "Threads", 1)
		set_optimizer_attribute(m, "TimeLimit", time_lapse)
		set_optimizer_attribute(m, "AbsGap", mingap / 2)
...
function global_OPT_DT_SG(X, Y, K, D, alpha, L_hat; lower = nothing, upper = nothing, eps = nothing, dtm_idx = nothing, w_sos = nothing, lambda = nothing, warm_start = nothing, mute = false, rlx = false, time = time_lapse)
	...
	T = 2^(D + 1) - 1
	Tb = Int(floor(T / 2))
	#eps = vec(mapslices(mini_dist, X, dims=2))
	#eps = min.(eps, minimum(lower.b)+1e-10)
	eps_min = minimum(eps)
	eps_max = maximum(eps)
	#println(eps)
	#println(eps_max)
	...
	#print(get_optimizer_attribute(m, "CPXPROB_LP"))
	# variables setup
	...
	#m, costs, costs_udt = var_costs(m, costs_idx, n)
	# constraints setup
	...
	optimize!(m)
	if termination_status(m) == MOI.INFEASIBLE #TODO 
	    println("Infeasible problem")
		return Tree(lower.a, lower.b, lower.c, lower.d, lower.z, D), 0, 0
	end
	#=
	if !mute
	...
	return tree, objv, gap #(gap>mingap)
end
...
##### obbt optimal function #####
function get_obbt_var(Var, varIdx, a, b, c, d)
	if Var == "a"
		obbt_var = a[varIdx]
	elseif Var == "d"
		obbt_var = d[varIdx]
...
function obbt_OPT(X, Y, K, D, alpha, L_hat, UB, obbt = "min", Var = "a", varIdx = 0; lower = nothing, upper = nothing,
	eps = nothing, dtm_idx = nothing, warm_start = nothing, mute = false)
	# parameter setup
	p, n = size(X)
	T = 2^(D + 1) - 1
	Tb = Int(floor(T / 2))
	...
	set_optimizer_attribute(m, "CPXPARAM_SolutionType", 2)

	# variables setup
	# ajt: bool, choose which feature to split; real, mixed feature(hyperplane) to split
	m, a = var_a(m, lower.a, a_udt, a_dt, warm_start.a, p, Tb, true)
	# bt: real, choose which value to split
	@variable(m, lower.b[t] <= b[t in 1:Tb] <= upper.b[t], start = warm_start.b[t])
	# dt: bool, 1 if t has a split, 0 otherwise
	m, d = var_d(m, lower.d, d_udt, d_dt, warm_start.d, Tb, true)
	# variables for leaf
	# ckt: from class 1 to k
	m, c = var_c(m, lower.c, c_udt, c_dt, warm_start.c, K, T, Tb, true)
	# zit: 1 if sample i in node t, t include both branch and leaf nodes.
	if z_udt isa Vector{Int64}# for only SG mode
		m, z = var_z_sg(m, z_udt, warm_start.z, n, T, Tb, true) # used in only SG mode
	else
		m, z = var_z(m, lower.z, z_udt, warm_start.z, n, T, Tb, true) # used in CF+SG mode
	end
	# variable for the loss
	@variable(m, 0 <= costs[i in 1:n] <= 1) # loss of sample i among all node 
	#m, costs, costs_udt = var_costs(m, Y, lower.c, upper.c, upper.z, z_udt, n, Tb)
	# constraints setup
	m = cons_setup_node(m, X, p, z_udt, Tb, eps, eps_max, a, b, d, z)
	m = cons_setup_leaf_SG(m, Y, n, K, T, Tb, c, z, z_udt, costs)

	# constraint on objective
	@constraint(m, 1 / L_hat * sum(costs[i] for i in 1:n) + alpha * sum(d[t] for t in 1:Tb) <= UB)

	obbt_var = get_obbt_var(Var, varIdx, a, b, c, d)
	if obbt == "min"
		@objective(m, Min, obbt_var)
	else # obbt == "max"
		@objective(m, Max, obbt_var)
	end
	optimize!(m)
	...
	if status == MOI.OPTIMAL
		...
		println("perform min has no solution $Var, $varIdx")
		objv = getfield(lower, Symbol(Var))[varIdx]
	end

	println("$obbt of $Var at $varIdx is $objv")
	return objv
end


function obbt_OPT_MILP(X, Y, K, D, alpha, L_hat, UB, obbt = "min", varIdx = 0;
	lower = nothing, upper = nothing, dtm_idx = nothing, warm_start = nothing, mute = false)
	# parameter setup
	p, n = size(X)
	T = 2^(D + 1) - 1
	Tb = Int(floor(T / 2))
	...
	m = cons_setup_leaf_MILP(m, Y, n, K, T, Tb, c, z_udt, costs)
	# constraint on objective
	@constraint(m, 1 / L_hat * sum(costs[i] for i in 1:n) + alpha * sum(lower.d) <= UB)

	obbt_var = get_obbt_var("c", varIdx, nothing, nothing, c, nothing)
	if obbt == "min"
		@objective(m, Min, obbt_var)
		optimize!(m)
		if result_count(m) <= 0
		...
		if result_count(m) <= 0
			println("perform min has no solution $Var, $varIdx")
			objv = getfield(upper, Symbol(Var))[varIdx]
		else
			objv = objective_bound(m) # objective_value(m) # using objective_bound so that the value is the smallest value
		end
	end
	println("$obbt of c at $varIdx is $objv")
	return objv
end
